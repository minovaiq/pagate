...
export default function ClientDetails() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center text-slate-900 dark:text-white">
        جاري التحميل...
      </div>
    );
  }
  ...
  if (!allowed || !client) {
    return (
      <div
        dir="rtl"
        className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-4 text-slate-900 dark:text-white"
      >
        <div className="bg-white dark:bg-slate-900 border border-red-300 dark:border-red-900 rounded-2xl p-4 text-center">
          <h1 className="text-lg font-black text-red-600 dark:text-red-400 mb-2">
            ليس لديك صلاحية
          </h1>
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-white p-3">
      <div className="max-w-[1400px] mx-auto space-y-2">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2 shadow-sm flex items-center justify-between text-slate-900 dark:text-white">
          <div>
            <h1 className="text-sm font-black">{client.full_name}</h1>

            <p className="text-[9px] text-slate-600 dark:text-slate-300 mt-1">
              {client.page_name || "-"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-white rounded-md px-2 h-7 text-[9px] font-black"
            >
              {darkMode ? "نهاري" : "ليلي"}
            </button>
            <button
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2 shadow-sm text-slate-900 dark:text-white">
          ...
          >
            <input
              ...
              className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md px-2 h-9 text-[10px] outline-none text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
          ...
          {transactions.length === 0 ? (
            ...
            <div className="space-y-1">
              {transactions.map((item) => (
                <div
                  key={item.id}
                  className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-2"
                >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <div className="text-left">
                      <h3 className="text-[10px] font-black text-green-600 dark:text-green-400">
...
function StatCard({ title, value, color }) {
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2 shadow-sm text-slate-900 dark:text-white">
      <p className="text-[8px] text-slate-600 dark:text-slate-300 mb-1">{title}</p>
...
function InfoBox({ title, value }) {
  return (
    <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-2">
      <p className="text-[8px] text-slate-600 dark:text-slate-300 mb-1">{title}</p>
...
function MiniBox({ title, value, color = "text-white", text = false }) {
  return (
    <div className="bg-slate-100 dark:bg-slate-900 rounded-md p-1">
      <p className="text-[8px] text-slate-600 dark:text-slate-300">{title}</p>
