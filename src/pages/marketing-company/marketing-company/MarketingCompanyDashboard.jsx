...
export default function MarketingCompanyDashboard({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`marketing-transactions-${project.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  async function loadTransactions() {
    try {
      ...
      const { data, error } = await supabase
        .from("transactions")
        .select("id,type,service_type,amount_received,company_profit,created_at,project_id")
        .eq("project_id", project.id);
      ...
      <div className="min-h-[40vh] flex items-center justify-center">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl px-6 py-5 flex flex-col items-center gap-3 shadow-2xl">
          ...
          <div className="text-center">
            <h2 className="text-sm font-black text-white">تحميل شركة الترويج</h2>
  ...
  return (
    <div dir="rtl" className="space-y-1.5 text-[11px]">
      <div className="h-8 flex items-center justify-between gap-2 border-b border-slate-800/70 pb-1">
        <div className="min-w-0 flex items-center gap-2">
          <h1 className="text-sm md:text-base font-black leading-none truncate">{project.name}</h1>
          <span className="text-slate-400 text-[9px] font-bold whitespace-nowrap">شركة الترويج</span>
        </div>

        <button
          onClick={loadTransactions}
          className="bg-slate-800 hover:bg-slate-700 rounded px-2 h-6 text-[9px] font-black shrink-0"
        >
          تحديث
        </button>
      </div>
      ...
      {showGoalModal && (
        ...
        >
          <form
            ...
            className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-xl p-4 shadow-2xl"
          >
            <div className="mb-4">
              <h2 className="text-sm font-black text-white">هدف هذا الشهر</h2>
              <p className="text-[10px] text-slate-300 mt-1">
            ...
            <input
              ...
              className="w-full h-10 bg-slate-950 border border-slate-700 rounded-lg px-3 text-sm font-black outline-none focus:border-amber-500"
            />
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              ...
              <button
                type="button"
                onClick={() => setShowGoalModal(false)}
                className="h-9 bg-slate-800 hover:bg-slate-700 rounded-lg text-[11px] font-black"
              >
                إلغاء
              </button>
            </div>
...
function StatCard({ title, subtitle, value, color }) {
  return (
    <div className="h-[92px] bg-slate-900 border border-slate-700/80 rounded-lg px-2.5 py-2 min-w-0 flex flex-col shadow-sm">
      <p className="text-slate-100 text-[10px] md:text-[11px] font-black leading-none truncate">
        {title}
      </p>
      ...
      <p className="text-slate-300 text-[8px] md:text-[9px] font-bold leading-tight mt-auto truncate">
        {subtitle}
      </p>
    </div>
  );
...
function GoalCard({ goal, remaining, progress, onEdit }) {
  return (
    <button
      type="button"
      onClick={onEdit}
      className="h-[92px] w-full text-right bg-slate-900 hover:bg-slate-800 border border-slate-700/80 hover:border-amber-500/60 rounded-lg px-2.5 py-2 min-w-0 transition-colors shadow-sm"
    >
      <div className="flex items-center justify-between gap-2 leading-none">
        <p className="text-slate-100 text-[10px] md:text-[11px] font-black truncate">
          هدف هذا الشهر
        </p>
        <span className="text-[8px] text-amber-300 font-black shrink-0">تعديل</span>
      </div>

      <div className="flex items-end justify-between gap-2 mt-2">
        <h2 className="text-[14px] md:text-[15px] font-black leading-none text-amber-400 tabular-nums truncate">
          {goal > 0 ? Number(goal).toLocaleString() : "غير محدد"}
        </h2>
        <span className="text-[9px] font-black text-amber-200 tabular-nums shrink-0">
      ...
      <div className="mt-1.5 flex items-center justify-between gap-2 min-w-0">
        <span className="text-[8px] md:text-[9px] font-bold text-sky-300 leading-none shrink-0">
          المتبقي
        </span>
        <span className="text-[11px] md:text-xs font-black text-sky-400 tabular-nums truncate leading-none">
